...
export function EvaluationPeriodFormDialog({
  ...
  useEffect(() => {
    if (open && editingItem) {
      console.log("Loading period data for id:", editingItem.id);
  ...
  async function loadPeriodData(id: string) {
    ...
    try {
      ...
      // API returns { header: {...}, periods: [...] }
      const header = response.header;
      const periodsList = response.periods;

      setYearId(String(header.yearId));
      setEvaluationTypeId(String(header.evaluationTypeId));
      setPeriodsCount(header.periodsCount);
      setPeriods(
        periodsList.map((p: any) => ({
          id: p.id,
          code: p.code,
          name: p.name || p.code, // Use code as name if name is not provided
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div className="grid gap-1.5">
                      <Label>
                        Año Académico <span className="text-destructive">*</span>
                      </Label>
                            ...
                            <tr key={period.code} className="border-t">
                              ...
                              <td className="px-3 py-2">
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    handleIsCurrentChange(index, !period.isCurrent)
                                  }
                                />
                              </td>
                            </tr>
