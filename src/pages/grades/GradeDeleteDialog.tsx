...
export function GradeDeleteDialog({ open, onOpenChange, grade, onSuccess }: GradeDeleteDialogProps) {
  ...
  const handleInitialDelete = async () => {
    ...
    try {
      ...
      const response = await apiClient.requestWithData<any>(`${ENDPOINTS.grades}/${grade.id}`, {
        method: "DELETE",
      });
      ...
      if (response.success) {
        ...
        // The API returns dependencies directly in the response
  ...
  const handleForceDelete = async () => {
    ...
    try {
      await gradesService.removeForce(grade.id);
      onSuccess();
      onOpenChange(false);
    } catch (err) {
      setError(err instanceof Error ? err.message : "No se pudo eliminar el registro.");
